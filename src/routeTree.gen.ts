/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from './routes/__root'
import { Route as QualificationsRouteImport } from './routes/qualifications'
import { Route as FypRouteImport } from './routes/fyp'
import { Route as IndexRouteImport } from './routes/index'

const QualificationsRoute = QualificationsRouteImport.update({
  id: '/qualifications',
  path: '/qualifications',
  getParentRoute: () => rootRouteImport,
} as any)
const FypRoute = FypRouteImport.update({
  id: '/fyp',
  path: '/fyp',
  getParentRoute: () => rootRouteImport,
} as any)
const IndexRoute = IndexRouteImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
  '/': typeof IndexRoute
  '/fyp': typeof FypRoute
  '/qualifications': typeof QualificationsRoute
}
export interface FileRoutesByTo {
  '/': typeof IndexRoute
  '/fyp': typeof FypRoute
  '/qualifications': typeof QualificationsRoute
}
export interface FileRoutesById {
  __root__: typeof rootRouteImport
  '/': typeof IndexRoute
  '/fyp': typeof FypRoute
  '/qualifications': typeof QualificationsRoute
}
export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths: '/' | '/fyp' | '/qualifications'
  fileRoutesByTo: FileRoutesByTo
  to: '/' | '/fyp' | '/qualifications'
  id: '__root__' | '/' | '/fyp' | '/qualifications'
  fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
  IndexRoute: typeof IndexRoute
  FypRoute: typeof FypRoute
  QualificationsRoute: typeof QualificationsRoute
}

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/qualifications': {
      id: '/qualifications'
      path: '/qualifications'
      fullPath: '/qualifications'
      preLoaderRoute: typeof QualificationsRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/fyp': {
      id: '/fyp'
      path: '/fyp'
      fullPath: '/fyp'
      preLoaderRoute: typeof FypRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexRouteImport
      parentRoute: typeof rootRouteImport
    }
  }
}

const rootRouteChildren: RootRouteChildren = {
  IndexRoute: IndexRoute,
  FypRoute: FypRoute,
  QualificationsRoute: QualificationsRoute,
}
export const routeTree = rootRouteImport
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()

import type { getRouter } from './router.tsx'
import type { startInstance } from './start.ts'
declare module '@tanstack/react-start' {
  interface Register {
    ssr: true
    router: Awaited<ReturnType<typeof getRouter>>
    config: Awaited<ReturnType<typeof startInstance.getOptions>>
  }
}
